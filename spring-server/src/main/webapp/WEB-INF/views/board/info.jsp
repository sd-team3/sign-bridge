        ...
        <c:if test="${board.categoryIdx == 'REPORT'}">
          <div class="report-detail-box">
            <div class="report-detail-row">
              ...
              <span>
                <c:choose>
                  <c:when test="${board.errorType == 'ACTION_RECOGNITION'}">동작 인식 오류</c:when>
                  <c:when test="${board.errorType == 'VIDEO_PLAYBACK'}">영상 재생 오류</c:when>
                  <c:when test="${board.errorType == 'TRANSLATION'}">번역 · 뜻풀이 오류</c:when>
                  <c:when test="${board.errorType == 'UI_BUG'}">화면 · 디자인 오류</c:when>
                  <c:when test="${board.errorType == 'ETC'}">기타</c:when>
                  <c:otherwise>${board.errorType}</c:otherwise>
                </c:choose>
              </span>
            </div>
            <c:if test="${not empty board.relatedWord}">
              <div class="report-detail-row"><span>관련 단어/기능</span><span>${board.relatedWord}</span></div>
            </c:if>
          </div>
        </c:if>
        <!-- 게시글 헤더 -->
