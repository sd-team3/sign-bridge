...
<title>SignBridge 관리자 - 단어 추가 요청 확인</title>
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    <div class="nav-group">
      ...
      <a class="nav-item" href="error-report.html">
        <span class="ni">🚨</span> 오류 신고
        <span class="badge">4</span>
      </a>
      <a class="nav-item active" href="word-request.html">
...
<div class="main">
  ...
  <div class="topbar">
    <div style="display:flex;align-items:center;gap:12px">
      ...
      <span class="topbar-title">단어 추가 요청 확인</span>
    </div>
    <span class="topbar-path">/admin/question/add</span>
  ...
  <!-- ══════════ PAGE: 단어 추가 요청 확인 ══════════ -->
  <div class="content page active" id="page-question-add">
...
// ═══════════ DATA ═══════════
const REQUESTS = [
  {id:'REQ-009',word:'배달',cat:'일상 생활',desc:'배달 음식 주문 시 자주 쓰일 것 같아요.',user:'김지수',date:'2025-06-29',status:'미처리'},
  {id:'REQ-008',word:'택배',cat:'일상 생활',desc:'택배 수령 시 필요합니다.',user:'강민서',date:'2025-06-27',status:'미처리'},
  {id:'REQ-007',word:'회의',cat:'직장',desc:'직장에서 자주 쓰는 단어입니다.',user:'오세진',date:'2025-06-25',status:'미처리'},
  {id:'REQ-006',word:'응급실',cat:'의료',desc:'의료 카테고리에 꼭 필요합니다.',user:'이민준',date:'2025-06-24',status:'미처리'},
];
  ...
  <div class="card" style="display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap">
    <div style="flex:1;min-width:240px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        <span class="pill pill-amber">미처리</span>
        <span class="td-mono" style="font-size:12px;color:var(--ink3)">${r.id}</span>
        <span style="font-size:12px;color:var(--ink3)">· ${r.date} · ${r.user}</span>
      </div>
      <div style="font-size:15px;font-weight:700;margin-bottom:4px">단어 추가 요청: "<b>${r.word}</b>" <span style="font-size:13px;font-weight:400;color:var(--ink3)">(${r.cat})</span></div>
      <div style="font-size:13px;color:var(--ink2)">${r.desc}</div>
    </div>
    <div style="display:flex;gap:6px;flex-shrink:0;align-items:center">
      <button class="topbar-btn btn-primary btn-sm" onclick="showToast('요청이 승인되었습니다 ✓','#2d9b6f')">✓ 승인</button>
      <button class="topbar-btn btn-ghost btn-sm" onclick="location.href='word-add.html'">직접 추가</button>
      <button class="topbar-btn btn-danger btn-sm" onclick="showToast('요청이 거절되었습니다','#c0392b')">✕ 거절</button>
    </div>
  </div>`).join('');
}
